...
│   └── sprites/actors.spd      # SpritePad actor pairs (right/left)
└── src/
    ├── main.c                  # Entry point, includes all modules
    ├── tileviewer.c            # Entry point for the scrollable tile viewer PRG
    ├── sprites/                # Hardware sprite loading and appearance mapping
    ├── visibility/             # LOS and 800-byte explored-cell bitset
...
main/src/sprites/
└── actor_sprites.c/.h           # SpritePad import, VIC RAM, frames and colours

main/src/visibility/
└── visibility.c/.h              # Bresenham LOS, current/remembered/hidden state
...
**Tile/sprite coordinates:**
Gameplay positions are map tiles; rendering converts them to 24x24 world
pixels. `TileViewOrigin` is the only camera-to-screen transform. It incorporates
the screen character origin and raw VIC X/Y fine scroll, and
`tiles_project_tile()` projects every actor from the same state. Do not add
per-actor corrections for camera fine scroll or the 38x24 border clip.
