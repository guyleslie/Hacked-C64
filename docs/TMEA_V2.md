# TMEA v2: Tile Metadata Extension Architecture
...
10. [Extending TMEA](#10-extending-tmea)
11. [Export Format](#11-export-format)
12. [Oscar64 Optimizations](#12-oscar64-optimizations)
...
```
main/src/mapgen/
├── tmea_types.h       (NEW) - Type definitions and enums
├── tmea_core.h        (NEW) - API declarations
└── tmea_core.c        (NEW) - Implementation
```

### 7.2 Include in main.c

Oscar64 requires including .c files in main.c:

```c
// main/src/main.c
#include "mapgen/tmea_core.c"        // MUST BE FIRST (other modules depend on it)
...
**Important:** `tmea_core.c` must be included FIRST because other modules (mapgen_utils.c, connection_system.c) use TMEA functions.

### 7.3 Update mapgen_types.h

Add TILE_MARKER constant (TILE_SECRET_DOOR removed in TMEA-first architecture):

```c
// main/src/mapgen/mapgen_types.h
...
// Value 6 reserved for future use (was TILE_SECRET_DOOR, now in TMEA metadata)
const unsigned char TILE_MARKER = 7;  // TMEA metadata marker
#define TILE_MASK   7
```

**Note:** `TILE_SECRET_DOOR = 6` has been removed. All doors now use `TILE_DOOR = 3` in compact_map, with door states (secret, locked, trapped, open/closed) managed via TMEA metadata. This eliminates redundancy and enables 5 door states instead of just 2.

### 7.4 Initialize in main()
...
int main(void) {
    ...
    // Initialize TMEA system (call once)
    init_tmea_system();
...
### 7.5 Reset Before Generation

```c
// main/src/mapgen/mapgen_utils.c
void reset_all_generation_data(void) {
    init_rnd();
    clear_map();

    // Reset room structures
    ...
    // Reset TMEA metadata for new generation (ADDED)
    reset_tmea_data();
...
**Note:** `reset_tmea_data()` is called automatically from `reset_all_generation_data()`, which is invoked before each dungeon generation. No manual call needed in `generate_level()`.

---

## 8. Usage Examples

### 8.1 Illusory Walls (Room + Corridor)
...
### 8.2 Secret Doors (TMEA-First Architecture)
...
### 9.2 Weighted Average Performance
...
### 10.1 Adding New Metadata Types
...
**TMEA v2** provides a robust, efficient, and extensible metadata system tailored specifically for the C64 dungeon generator project. Its hybrid room+global architecture optimizes for the project's room-based generation while supporting corridor and map-wide features.
...
2. Test with simple features (illusory walls)
3. Gradually add complexity (traps, entities)
